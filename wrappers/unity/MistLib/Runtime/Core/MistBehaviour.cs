...
namespace MistLib
{
    ...
    public abstract class MistBehaviour : MonoBehaviour
    {
        ...
        {
            if (SyncObject.IsOwner) LocalStart();
            else RemoteStart();
        }

        protected virtual void Update()
        {
            if (SyncObject.IsOwner) LocalUpdate();
            else RemoteUpdate();
        }

        protected virtual void FixedUpdate()
        {
            if (SyncObject.IsOwner) LocalFixedUpdate();
            else RemoteFixedUpdate();
        }

        protected virtual void OnDestroy()
        {
            if (SyncObject.IsOwner) LocalOnDestroy();
            else RemoteOnDestroy();
        }
