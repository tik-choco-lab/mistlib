...
なると破綻するため、mistlib は各ノードの接続数に上限を設けたうえで、3D 空間上の近さから
「どのピアと繋いでおくか」を決める。AOI 内の近いピアに加え、各方向の最近傍を1つずつ確保
することで、どの方向も途切れないようにする。直接繋がっていないピア宛のメッセージは、
こうしてできたオーバーレイ上を中継される。
